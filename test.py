...
ok_userjson = eval(json.dumps(uesr_text[0]))# print(ok_userjson)
down_url = "https://osu-stats-signature.vercel.app/card?user=" + str(
    ok_userjson['user_id']) + "&mode=std&blur=6&w=1920&h=1117"#
下载地址合成
if bottok['proxy'] == True:
    proxies = {
        'http': 'socks5://127.0.0.1:8089',
...
cairosvg.svg2png(url = './tmp/osu/' + str(ok_userjson['user_id']) +
    '.svg', write_to = './tmp/osu/' + str(ok_userjson['user_id']) +
    '.png')
...
x = {}
i = 0
for index_a in index_array: #index_a = int(index_a)
...
inpu_osuinfo(tg_sql_id, id, osu_now_level, osu_now_pp)# -- -- -- -- --
    -- -- -- -- -- -- --
    down_url = "https://osu-stats-signature.vercel.app/card?user=" +
    str(ok_userjson['user_id']) + "&mode=" + str(mode) +
    "&blur=6&w=1920&h=1117"#
下载地址合成
if bottok['proxy'] == True:
    proxies = {
        'http': 'socks5://127.0.0.1:8089',
...
cur = osu.cursor()
sql =
    "CREATE TABLE IF NOT EXISTS osuinfo(telid INTEGER PRIMARY KEY,osuid INTEGER,level INTEGER,pp INTEGER)"
cur.execute(sql)
...
sence = cur.fetchall()
if oid == 1:
    for i in range(len(sence)): #TG取OSU ID# print(sence[i][0])
if str(sence[i][0]) == teleid:
    return sence[i][1]
elif oid == 0:
    for i in range(len(sence)): #OSU ID取 TG# print(sence[i][0])
if str(sence[i][1]) == teleid:
    return sence[i][0]# 写的头疼（
elif oid == 2:
    for i in range(len(sence)): #OSU ID取OSU ID# print(sence[i][0])
if str(sence[i][1]) == teleid:
    return sence[i][1]
elif oid == 3:
    for i in range(len(sence)): #TG取TG# print(sence[i][0])
if str(sence[i][0]) == teleid:
    return sence[i][0]
elif oid == 4:
    for i in range(len(sence)): #TG取OSU 等级# print(sence[i][0])
if str(sence[i][1]) == teleid:
    return sence[i][2]
elif oid == 5:
    for i in range(len(sence)): #TG取 PP# print(sence[i][0])
if str(sence[i][1]) == teleid:
    return sence[i][3]
else :
    return False
...
uesr_text = json.loads(res.text)# json解析
ok_userjson = eval(json.dumps(uesr_text[0]))# print(ok_userjson)
down_url = "https://a.ppy.sh/" + str(ok_userjson['user_id']) +
    "?img.jpeg"#
下载地址合成
if bottok['proxy'] == True:
    proxies = {
...
im1 = Image.open('./tmp/osu/' + str(ok_userjson['user_id']) + '.png')# im1
    .thumbnail((700, 400))
im.paste(im1, (279, 184))
im.save('./tmp/osu/' + str(ok_userjson['user_id']) + 'ok' + '.png')
bk_img = cv2.imread('./tmp/osu/' + str(ok_userjson['user_id']) + 'ok' +
    '.png')# 设置需要显示的字体
fontpath = "./font/hyl.ttf"
font = ImageFont.truetype(fontpath, 40)
img_pil = Image.fromarray(bk_img)
draw = ImageDraw.Draw(img_pil)# 绘制文字信息
draw.text((262, 510), str(ok_userjson['username']), font = font, fill =
    (255, 255, 255))# 名字
draw.text((218, 601), str(ok_userjson['level']), font = font, fill =
    (255, 255, 255))# 等级
draw.text((196, 675), str(ok_userjson['pp_raw']), font = font, fill =
    (255, 255, 255))# PP
bk_img = np.array(img_pil)
cv2.imwrite('./tmp/osu/' + str(ok_userjson['user_id']) + '.png',
    bk_img)
...
def ping_host(ip):
    ip_address = ip
response = ping(ip_address)# print(response)
...
for i in range(10):
    hot_news = ''#
将信息保存到字典中
hot_news = str(i + 1) + '.' + urls_titles[i + 1].get_text() + '|热度：' +
    hotness[i].get_text()
...
def send_jrrp(message):
    try:
    if fill_json(message.from_user.id) == False: #print('创建用户储存文件')
...
bot.reply_to(message, "你今天的人品是：" + str(jrrpp) + '\n' + jrrp_text(
    ...
    with open('./user/jrrp/' + str(message.from_user.id) + '.json',
        'r') as timejrrp:
...
bot.reply_to(message, "你今天的人品是：" + str(bottok['jrrp']) + '\n' +
    ...
    file = open('./user/jrrp/' + str(message.from_user.id) + '.json',
        'w')
jrrpp = get_random()
data1 = {
    'jrrp': jrrpp,
...
sti = open(path_file_name[random.randint(0, len(path_file_name) - 1)],
    'rb')
...
if ycxt_json(message.from_user.id) == False: #print(ycxt_json(message
    .from_user.id))
file = open('./user/ycxt/' + str(message.from_user.id) + '.json', 'w',
    encoding = 'utf-8')
guuu = get_phi()
data_gu = {
    'impress': 100,
...
html = page.text# print(source1)# 从字符串解析
element = etree.HTML(html)
# 元素列表， 获取的方式列出了如下两种# ps = element.xpath(
...
index.append(index1)
re_text = PP(index, text)# 对字典性数据按key进行排序， 即key = lambda re: re[0]， 排序完成后再转换为字典型数据
...
bot.edit_message_text(str(pingipp) + ' 的延迟为: \n\n' + str(ping_host(
    str(pingipp))) + ' ms', chatjson.chat.id, chatjson.message_id)# print(
    output_str)
else :
    bot.send_chat_action(message.chat.id, 'typing')
...
out = osu_user_outinfo(str(out_osuinfo(str(message.from_user.id), 1)),
    "std")
chatjson_img = bot.edit_message_text("正在上传图片请稍后....", chatjson_img.chat
    .id, chatjson_img.message_id)
bot.send_chat_action(message.chat.id, 'upload_photo')
...
bot.delete_message(chatjson_img.chat.id, chatjson_img.message_id)
    ...
    bot.send_chat_action(message.chat.id, 'typing')
bot.edit_message_text('上传时出错了惹...\n错误日志: ' + str(gubot), chatjson_img
    .chat.id, chatjson_img.message_id)
time.sleep(3)
bot.delete_message(chatjson_img.chat.id, chatjson_img.message_id)
else :
    try:
...
bot.edit_message_text('呜呜呜...咕小酱遇到了严重问题......\n错误日志: ' + str(boterr),
    chatjson_img.chat.id, chatjson_img.message_id)
time.sleep(3)
bot.delete_message(chatjson_img.chat.id, chatjson_img.message_id)
@ bot.message_handler(commands = ['guosubind'])
def send_osuinfo(message):
    outtext = howpingip(message.text)# print(outtext)# 指令输出
if outtext == False:
    bot.send_chat_action(message.chat.id, 'typing')
bot.reply_to(message,
    "呜呜呜....你光发指令干嘛 OSU 用户名呢???\n(缺少参数/guosubind [OSU用户名/OSU ID])"
)
else :
    try:
...
if inpu_osuinfo(message.from_user.id, osuid['user_id'], osuid['level'],
        osuid['pp_raw']) == True:
    bot.send_chat_action(message.chat.id, 'typing')
bot.edit_message_text('绑定 OSU 用户名成功啦!', chatjson_sql.chat.id,
    chatjson_sql.message_id)
else :
    bot.send_chat_action(message.chat.id, 'typing')
...
bot.reply_to(message, "你的Telegram ID 绑定了 OSU 用户名了的说...")
    ...
    bot.send_chat_action(message.chat.id, 'typing')
bot.reply_to(message, "此 OSU 用户名不存在或者数据库处理出错的说...")
except Exception as ooo: #inpu_osuinfo(message.from_user.id, osuid)
bot.send_chat_action(message.chat.id, 'typing')
bot.reply_to(message, '呜呜呜...咕小酱遇到了严重问题......\n错误日志: ' + str(ooo))
# 数据库读取函数后面参数 ? (自造)# #0 OSU ID 取 Telegeam ID
...
osu.close()
bot.send_chat_action(message.chat.id, 'typing')
bot.edit_message_text('OSU 用户名解绑完成!', chatjson_del.chat.id,
    chatjson_del.message_id)
...
zhihu_text = '知乎热搜:\n'
for i in range(0, 10):
    zhihu_text += str(i + 1) + '.『' + str(zhihu_json_list[i]['name']) +
    '』 - ' + str(zhihu_json_list[i]['query']) + '\n'
bot.edit_message_text(zhihu_text, zhihu_text_go.chat.id,
    zhihu_text_go.message_id)# bot.reply_to(message, zhihu_text)
except:
    bot.edit_message_text('呜呜呜....运行错误', zhihu_text_go.chat.id,
        zhihu_text_go.message_id)
@ bot.message_handler(commands = ['gubili'])
def send_bilibili(message):
    try:
    bot.send_chat_action(message.chat.id, 'typing')
bili_json_input = requests.get(
    "https://hibiapi.aliserver.net/api/bilibili/v3/video_ranking"
)
if bili_json_input.status_code == 200:
    try:
    bili_text_go = bot.reply_to(message, '正在获取请稍后...')
bili_json_list = json.loads(bili_json_input.content)['rank']['list']# print(
    bili_json_list)
except:
    pass# bili_json_out = eval(json.dumps(json.loads(bili_json_input.text)))# print(
        bili_json_out)# bili_json_list = bili_json_out['rank']['list']
bili_text = 'BiliBili热门视频排行榜Top10:\n'
for i in range(0, 10):
    bili_text += str(i + 1) + '.『' + str(bili_json_list[i]['title']) +
    '』 | UP主:' + str(bili_json_list[i]['author']) + '\n'
bot.edit_message_text(bili_text, bili_text_go.chat.id, bili_text_go.message_id)# bot
    .reply_to(message, bili_text)
except Exception as eeer:
    bot.edit_message_text('呜呜呜....运行错误', bili_text_go.chat.id,
        bili_text_go.message_id)

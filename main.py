...
if not os.path.exists("./config.yml"):  # 初始化Bot
    ...
    if bot_config["botToken"] is None:
        logger.error(f"配置文件 botToken 未填写")
        sys.exit()
    elif bot_config["osuToken"] is None:
        logger.error(f"配置文件 osuToken 未填写")
        sys.exit()
    elif bot_config["apikey"] is None:
        logger.error(f"配置文件 apikey 未填写")
        sys.exit()
    elif bot_config["musicapi"] is None:
        logger.error(f"配置文件 musicapi 未填写")
        sys.exit()
    elif bot_config["musicphone"] is None:
        logger.error(f"配置文件 musicphone 未填写")
        sys.exit()
    elif bot_config["musicpwd"] is None:
...
def sc_netmusic(name):  # 网易云音乐搜索
    try:
        ...
        son_art_text = ''
        out_list = []
        for gu_i in range(0, len(son_name)):
            son_name_out = son_name[gu_i]["name"]
            son_name_art_inp = son_name[gu_i]["artists"]
            son_id_out = son_name[gu_i]["id"]
...
def dl_netmusic_info(mid):  # 网易云音乐信息获取
    try:
        ...
        son_art_text = ''
        for i_gu in range(0, len(son_art_json)):
            if len(son_art_json) == 0:
                son_art_text += son_info_json["songs"][0]["ar"][i_gu]["name"]
            else:
                if i_gu == len(son_art_json) - 1:
                    son_art_text += son_info_json["songs"][0]["ar"][i_gu]["name"]
                else:
                    son_art_text += son_info_json["songs"][0]["ar"][i_gu]["name"] + ","
        jsonout = {
...
def dl_netmusic(mid):  # 网易云音乐下载!
    try:
        denlu = requests.get(str(bot_config["musicapi"]) + "/login/cellphone?phone=" + str(
            bot_config["musicphone"]) + "&password=" + str(bot_config["musicpwd"]))  # 获取登录信息
        ...
        if outjson["data"][0]["url"] is None:  # 检测音乐地址
            return False
        res = requests.get(outjson["data"][0]["url"])
        with open("./tmp/" + str(outjson["data"][0]["id"]) + '.' + str(outjson["data"][0]["type"]), 'wb') as gu_f:
            gu_f.write(res.content)
        time.sleep(0.5)
        son_info_get = requests.get(str(bot_config["musicapi"]) + "/song/detail?ids=" + str(mid))
        son_info_json = json.loads(json.dumps(json.loads(str(son_info_get.text))))
        son_name = son_info_json["songs"][0]["name"]
        ...
        son_art_json = son_info_json["songs"][0]["ar"]
        son_art_text = ''
        for gugu_i in range(0, len(son_art_json)):
            if len(son_art_json) == 0:
                son_art_text += son_info_json["songs"][0]["ar"][gugu_i]["name"]
            else:
                if gugu_i == len(son_art_json) - 1:
                    son_art_text += son_info_json["songs"][0]["ar"][gugu_i]["name"]
                else:
                    son_art_text += son_info_json["songs"][0]["ar"][gugu_i]["name"] + ","
        if str(outjson["data"][0]["type"]) == "mp3":
            ...
            with open("./tmp/" + str(mid) + '.jpg', 'wb') as gu_f:
                gu_f.write(dl_img.content)
            # 音乐信息写入
...
def nbnhhsh(text):
    ...
    try:
        ...
        for i_loop in range(0, len(sc)):
            if len(sc) != i_loop:
                gu_text += str(i_loop + 1) + '.『' + ok_json[0]["trans"][i_loop] + '』\n'
            else:
                gu_text += str(i_loop + 1) + '.『' + ok_json[0]["trans"][i_loop] + '』'
        return gu_text
    except:
...
def fill_json():  # 让我康康你的数据文件在不在?
    if os.path.isfile('./user/jrrp/' + str(id) + '.json'):
        return True
    else:
        return False
...
def dl_sdmusic(url_dl):
    try:
        ...
        try:
            ...
            audiofile.tag.images.set(3, open("./tmp/" + page_text["vidTitle"] + '.jpg', 'rb').read(),
                                     'image/jpeg')  # 添加封面
            ...
            audiofile.initTag()  # 初始化所有标签信息，将之前所有的标签清除
            audiofile.tag.images.set(3, open("./tmp/" + getStrAsMD5(page_text["vidTitle"]) + '.jpg', 'rb').read(),
                                     'image/jpeg')  # 添加封面
            audiofile.tag.album = u"SoundCloud"  # 唱片集
            audiofile.tag.title = page_text["vidTitle"]  # 标题
            audiofile.tag.save()  # 保存文件
...
def is_sd_url(url):
    if re.match(r'^https*://(www.)*soundcloud\.com/', url) is None:
        return False
    else:
        return True
...
def is_163pc_url(url):
    if re.match(r'^https*://(music.)*163\.com/', url) is None:
        return False
    else:
        return True
...
def is_163ph_url(url):
    if re.match(r'^https*://(y.music.)*163\.com/', url) is None:
        return False
    else:
        return True
...
def send_jrrp(message):
    try:
        ...
        bot.send_chat_action(message.chat.id, 'typing')
        if os.path.isfile('./user/jrrp/' + str(message.from_user.id) + '.json'):
...
def gudlsoundcloud(message):
    global chatjson_img
    if not howpingip(message.text):
        bot.send_chat_action(message.chat.id, 'typing')
        bot.reply_to(message, "呜呜呜...指令有问题\n(指令格式 /gudlsds [SoundCloud音乐链接] 暂不支持列表下载)")
    else:
        try:
            text_rl = howpingip(message.text)
            if is_sd_url(text_rl):
                bot.send_chat_action(message.chat.id, 'typing')
                chatjson_img = bot.reply_to(message, "正在从SoundCloud获取音乐信息请稍后....")
                info_music = dl_sdmusic_info(text_rl)
                chatjson_img = bot.edit_message_text("正在下载『" + info_music + "』请稍后....", chatjson_img.chat.id,
                                                     chatjson_img.message_id)
                dl_muss = dl_sdmusic(text_rl)
                chatjson_img = bot.edit_message_text("正在上传『" + info_music + "』请稍后....", chatjson_img.chat.id,
                                                     chatjson_img.message_id)
            ...
            # print(boterr)
            bot.send_chat_action(message.chat.id, 'typing')
            bot.edit_message_text('呜呜呜...咕小酱遇到了严重问题......\n错误日志: ' + str(boterr), chatjson_img.chat.id,
                                  chatjson_img.message_id)
            time.sleep(3)
            bot.delete_message(chatjson_img.chat.id, chatjson_img.message_id)
    ...
    if not howpingip(message.text):
        ...
        try:
            ...
            try:
                ...
                chatjson_img = bot.send_photo(message.chat.id, "https://http.cat/404",
...
def gudlwyy(message):
    ...
    if howpingip(message.text) == False:
        ...
        try:
            ...
            chatjson_img = bot.edit_message_text(
                "正在下载『" + info_musi_info['muname'] + ' - ' + info_musi_info['art'] + "』请稍后....", chatjson_img.chat.id,
                chatjson_img.message_id)
            info_music = dl_netmusic(text_ok_id)
            if info_music['success'] == "err":
                chatjson_img = bot.edit_message_text("抱歉,咕小酱下载音乐时候遇到了未知错误,请重新尝试....", chatjson_img.chat.id,
                                                     chatjson_img.message_id)
                return None
            chatjson_img = bot.edit_message_text(
                "正在上传『" + info_musi_info['muname'] + ' - ' + info_musi_info['art'] + "』请稍后....", chatjson_img.chat.id,
                chatjson_img.message_id)
            bot.send_chat_action(message.chat.id, 'upload_audio')
            audio = open(info_music['fil'], 'rb')
            bot.send_audio(message.chat.id, audio, performer=info_music['art'], title=info_music['muname'],
                           reply_to_message_id=message.message_id)
            bot.send_chat_action(message.chat.id, 'typing')
            bot.edit_message_text("『" + info_music['muname'] + ' - ' + info_music['art'] + '』上传完成!',
...
def callback_handle(call):
    ...
    if out_json["do"] == "sc":  # 网易云音乐搜索
        ...
        info_musi_info = dl_netmusic_info(text_ok_id)
        chatjson_img = bot.edit_message_text(
            "正在下载『" + info_musi_info['muname'] + ' - ' + info_musi_info['art'] + "』请稍后....", musiout.chat.id,
            musiout.message_id)
        info_music = dl_netmusic(text_ok_id)
        if info_music['success'] == "err":
            chatjson_img = bot.edit_message_text("抱歉,咕小酱下载音乐时候遇到了未知错误,请重新尝试....", chatjson_img.chat.id,
        ...
        audio = open(info_music['fil'], 'rb')
        bot.send_audio(call.message.chat.id, audio, performer=info_music['art'], title=info_music['muname'],
                       reply_to_message_id=out_json["ud"])
        bot.send_chat_action(call.message.chat.id, 'typing')
        bot.edit_message_text("『" + info_music['muname'] + ' - ' + info_music['art'] + '』上传完成!', chatjson_img.chat.id,
                              chatjson_img.message_id)
        time.sleep(3)
        bot.delete_message(chatjson_img.chat.id, chatjson_img.message_id)
        audio.close()
...
def guscwyy(message):
    if howpingip(message.text) == False:
        ...
        for i in range(0, 5):
            ...
            mkjson = '{"id":' + str(out_sc_list[i]["sonid"]) + ',"cd":' + str(message.chat.id) + ',"ud":' + str(
                message.message_id) + ',"do":"sc\"}'
